...
import { Wallet as WalletIcon, Plus, ArrowUpRight, Calculator, Check, PhoneCall, DollarSign } from 'lucide-react'
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="p-6 rounded-xl bg-blue-600 text-white shadow-sm flex flex-col justify-between border border-blue-700">
          <div>
            <div className="flex items-center justify-between text-blue-100 mb-2">
              <span className="text-xs font-bold uppercase tracking-wider">PRIMARY CLIENT WALLET</span>
        ...
        <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between text-slate-500 mb-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600">VOIP Call Rate</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Estimated Total Calls</label>
                <input
                  type="number"
                  value={calcCalls}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Avg. Call Duration (minutes)</label>
                <input
                  type="number"
                  step="0.1"
                  value={calcAvgMins}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Rate per Minute ($)</label>
                <input
                  type="number"
                  step="0.001"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Concurrent Lines Needed</label>
                <input
                  type="number"
          ...
          <div className="lg:col-span-5 bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col justify-between space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-3">Calculation Output</h3>
              
              <div className="space-y-3 text-xs">
                <div className="flex justify-between text-slate-700">
                  <span className="font-medium">Estimated Total Minutes:</span>
                  <span className="font-bold text-slate-900 font-mono">{estimatedTotalMins.toLocaleString()} mins</span>
                </div>
                <div className="flex justify-between text-slate-700">
                  <span className="font-medium">Target Concurrency:</span>
                  <span className="font-bold text-slate-900 font-mono">{calcConcurrency} lines</span>
                </div>
                <div className="border-t border-slate-200 pt-3 flex justify-between items-center">
                  <span className="font-bold text-slate-900 text-xs">Required VOIP Balance:</span>
                  <span className="text-2xl font-black text-blue-600 font-mono">${calculatedVoipNeed}</span>
                </div>
              </div>
            </div>
      ...
      <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs">
        <h2 className="text-base font-bold text-slate-900 mb-4">Wallet Transactions History</h2>
            ...
            <tbody className="divide-y divide-slate-100">
              {wallet.transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50 transition">
                  <td className="p-3 text-slate-600 font-mono font-medium">{tx.date}</td>
                  <td className="p-3">
                    <span className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${
                      tx.type === 'Credit' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-blue-100 text-blue-700 border border-blue-200'
                    }`}>
                      {tx.type}
                    </span>
                  </td>
                  <td className="p-3 text-slate-800 font-semibold">{tx.description}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 border border-slate-200 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleTopUpSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Select Amount ($)</label>
