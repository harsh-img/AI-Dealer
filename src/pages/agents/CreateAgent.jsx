...
const CreateAgent = () => {
  ...
  const agent = isEdit ? (agentsData.agents || []).find((a) => String(a.id) === String(id)) : null

  // If edit mode and agent not found
  if (isEdit && !agent) {
    return (
    ...
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 pb-10">
      ...
      <div className="space-y-2 pb-6 border-b border-slate-200">
        <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
          ...
          {isEdit ? (
            <>
              <Link to={`/agents/${agent.id}`} className="hover:text-indigo-600 transition">
                {agent.name}
              </Link>
              <span>/</span>
            ...
            <span className="text-slate-800">Create New Agent</span>
          )}
        </div>
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {isEdit
              ? 'Update prompt instructions, voice persona, language, or operational status.'
              : 'Configure a new AI voice persona with custom instructions, language, and tone.'}
          </p>
        </div>
      </div>

      {/* Unified Form for Create & Edit */}
      <AgentForm initialValues={agent} isEdit={isEdit} />
    </div>
  )
}
