...
export const AppProvider = ({ children }) => {
  ...
  const [wallet, setWallet] = useState({
    ...
    ratePerMin: 0.015,
    transactions: [
      { id: 'tx-1', type: 'Credit', amount: 500.00, description: 'Self Top-Up (Card ****4242)', date: '2026-03-01 14:20' },
      { id: 'tx-2', type: 'Transfer to VOIP', amount: 250.00, description: 'VOIP Trunk Allocation', date: '2026-03-02 09:15' }
    ]
  })
  ...
  const [aiAgents, setAiAgents] = useState([
    {
      ...
      prompt: 'You are Sarah, a warm and professional outbound SDR for SaaS products. Qualify prospects by asking about team size and current workflow friction.',
      gender: 'Female',
      language: 'English (US)',
    ...
    },
    {
      ...
      prompt: 'You are David, an expert Real Estate qualifier. Ask prospects if they are looking to buy or sell within the next 90 days.',
      gender: 'Male',
      language: 'English (US)',
    ...
    },
    {
      id: 'agent-3',
      name: 'Frida Name',
      prompt: 'You are Frida, an empathetic survey and marketing agent collecting user feedback for Q3 marketing leads.',
      gender: 'Female',
      language: 'Spanish',
      accent: 'Castilian',
      speed: 0.95,
  ...
  const updateAiAgent = (agentId, updatedFields) => {
    setAiAgents(prev => prev.map(ag => ag.id === agentId ? { ...ag, ...updatedFields } : ag))
  }
    ...
    setCampaigns(prev => prev.map(c => {
      if (c.id === campId) {
        const logEntry = {
          ...
          status: c.telegram?.enabled ? 'Sent Successfully' : 'Disabled (Skipped)',
